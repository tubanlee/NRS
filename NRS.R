...
rqcor1<-function (x, y = NULL) 
{
  ...
  meanx <- rqmean(x)
  meany <- rqmean(y)
  amean <- (x - meanx[1])/omhatx[1]
  bmean <- (y - meany[1])/omhaty[1]
  aetm <- (x - meanx[2])/omhatx[2]
  betm <- (y - meany[2])/omhaty[2]
  arm <- (x - meanx[3])/omhatx[3]
  brm <- (y - meany[3])/omhaty[3]
  aqm <- (x - meanx[4])/omhatx[4]
  bqm <- (y - meany[4])/omhaty[4]
  productmean<-amean*bmean
  productetm<-aetm*betm
  productrm<-arm*brm
  productqm<-aqm*bqm
  mean1<-mean(productmean)
  etm1<-rqmean(productetm)[2]
  rm1<-rqmean(productrm)[3]
  qm1<-rqmean(productqm)[4]
  rqcor <- c(mean1=mean1,etm1=etm1,rm1=rm1,qm1=qm1)
  return(rqcor)
}
rqcor<-function (x,y=NULL,interval=9,fast=TRUE,batch="auto",standist=c("exponential","Rayleigh","exp","Ray")){
  matrix1 <- cbind(x, y)
  matrix1 <- na.omit(matrix1)
  x <- matrix1[, 1]
  y <- matrix1[, 2]
  ...
  mean1<-(meanxy[1]-meanx[1]*meany[1])/(sqrt(meanx2[1]-meanx[1]^2)*sqrt(meany2[1]-meany[1]^2))
  etm1<-(meanxy[2]-meanx[2]*meany[2])/(sqrt(meanx2[2]-meanx[2]^2)*sqrt(meany2[2]-meany[2]^2))
  rm1<-(meanxy[3]-meanx[3]*meany[3])/(sqrt(meanx2[3]-meanx[3]^2)*sqrt(meany2[3]-meany[3]^2))
  qm1<-(meanxy[4]-meanx[4]*meany[4])/(sqrt(meanx2[4]-meanx[4]^2)*sqrt(meany2[4]-meany[4]^2))
  rqcor <- c(mean1=mean1,etm1=etm1,rm1=rm1,qm1=qm1)
  return(rqcor)
}
...
x<-c(xy[,1])
y<-c(xy[,2])
rqcor(x=x, y=y,interval=9,fast=TRUE,batch="auto",standist="Ray")
rqcor(x=x, y=y,interval=9,fast=TRUE,batch="auto",standist="exp")
...
#gamma
xy<-rcor(n = 5130, xparamater1 = 0.2, xparamater2 = 1, yparamater1 = 0.2, yparamater2 = 1, correlation = 0.8,FUN1=qgamma,FUN2=qgamma)
x<-c(xy[,1])
y<-c(xy[,2])
rqcor(x=x, y=y,interval=9,fast=TRUE,batch="auto",standist="Ray")
rqcor(x=x, y=y,interval=9,fast=TRUE,batch="auto",standist="exp")

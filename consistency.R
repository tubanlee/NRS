...
for(i in (1:50)){
  ...
  scale1<-Bias_rqscale(x,target1=2-(pi/2),target2=0.36707,interval=9,fast=TRUE,batch=1000,sorted=TRUE)
  all<-c(tmBias=tm1Bias,wmBias=wm1Bias,qmBias=qm1Bias,rmBias=rm1Bias,sqsd=scale1[1],srsd=scale1[2],sql2=scale1[3],srl2=scale1[4])
...
for (a in (1:300)) {
  allforweibull<-c()
  for(i in (1:1)){
    x<-c(rweibull(5400, shape=a/50, scale = 1))
    targetwei<-lmrwei(para = c(0, 1, a/50), nmom = 3)
    x<-sort(x,decreasing = FALSE,method ="radix")
    tm1<-mean(x,1/9)
    wm1<-mean(winsor(x,fraction=1/9))
    qm1<-qm(x,interval=9,fast=TRUE,batch=1000,d=0.821497,sorted=TRUE)
    rm1<-rm(x,interval=9,fast=TRUE,batch=10000,d=0.366919)
    tm1Bias<-((((tm1))-gamma(1+1/(a/50))))/sd(x)  
    wm1Bias<-((((wm1))-gamma(1+1/(a/50))))/sd(x)
    qm1Bias<-((((qm1))-gamma(1+1/(a/50))))/sd(x)  
    rm1Bias<-((((rm1))-gamma(1+1/(a/50))))/sd(x)
    scale1<-Bias_rqscale(x,target1=(gamma(1+2/(a/50))-(gamma(((1+1/(a/50)))))^2),target2=targetwei[2],interval=9,fast=TRUE,batch=1000,sorted=TRUE)
    all<-c(tmBias=tm1Bias,wmBias=wm1Bias,qmBias=qm1Bias,rmBias=rm1Bias,sqsd=scale1[1],srsd=scale1[2],sql2=scale1[3],srl2=scale1[4])
...
for (a in (1:300)) {
  allforgamma<-c()
  for(i in (1:1)){
    x<-c(rgamma(5400, shape=a/50, rate = 1))
    targetgam<-lmrgam(para = c(a/50, 1), nmom = 3)
    x<-sort(x,decreasing = FALSE,method ="radix")
    tm1<-mean(x,1/9)
    wm1<-mean(winsor(x,fraction=1/9))
    qm1<-qm(x,interval=9,fast=TRUE,batch=1000,d=0.821497,sorted=TRUE)
    ...
    scale1<-Bias_rqscale(x,target1=a/50,target2=targetgam[2],interval=9,fast=TRUE,batch=1000,sorted=TRUE)
    all<-c(tmBias=tm1Bias,wmBias=wm1Bias,qmBias=qm1Bias,rmBias=rm1Bias,sqsd=scale1[1],srsd=scale1[2],sql2=scale1[3],srl2=scale1[4])
...
for (a in (1:300)) {
  allforlnorm<-c()
  for(i in (1:1)){
    x<-c(rlnorm(5400,meanlog=0,sdlog=a/50))
    targetlnorm<-lmrln3(para = c(0,0, a/50), nmom = 3)
    x<-sort(x,decreasing = FALSE,method ="radix")
    tm1<-mean(x,1/9)
    wm1<-mean(winsor(x,fraction=1/9))
    qm1<-qm(x,interval=9,fast=TRUE,batch=1000,d=0.821497,sorted=TRUE)
    ...
    scale1<-Bias_rqscale(x,target1=(exp((a/50)^2)*(-1+exp((a/50)^2))),target2=targetlnorm[2],interval=9,fast=TRUE,batch=1000,sorted=TRUE)
    all<-c(tmBias=tm1Bias,wmBias=wm1Bias,qmBias=qm1Bias,rmBias=rm1Bias,sqsd=scale1[1],srsd=scale1[2],sql2=scale1[3],srl2=scale1[4])
...
for (a in (1:300)) {
  ...
  for(i in (1:1)){
    library(VGAM)
    x<-c(VGAM::rpareto(5400, scale  = 1, shape=2+a/50))
    targetlpareto<-lmrgpa(para = c(1,1/(2+a/50),- 1/(2+a/50)), nmom = 3)
    x<-sort(x,decreasing = FALSE,method ="radix")
    tm1<-mean(x,1/9)
    wm1<-mean(winsor(x,fraction=1/9))
    qm1<-qm(x,interval=9,fast=TRUE,batch=1000,d=0.821497,sorted=TRUE)
    ...
    scale1<-Bias_rqscale(x,target1=((2+a/50))*(1)/((-2+(2+a/50))*((-1+(2+a/50))^2)),target2=targetlpareto[2],interval=9,fast=TRUE,batch=1000,sorted=TRUE)
    all<-c(tmBias=tm1Bias,wmBias=wm1Bias,qmBias=qm1Bias,rmBias=rm1Bias,sqsd=scale1[1],srsd=scale1[2],sql2=scale1[3],srl2=scale1[4])

...
#this finite sample consistency is just a qualitative evaluation, since equinterval simulation is biased, but enough to show that 
#when sample size <30, the bias is very large, to ensure consistency >0.9, sample size >30 is required.
...
allforlaplace<-foreach(i = c(seq(from=9, to=108, by=1),seq(from=117, to=1305, by=108),seq(from=1413, to=5733, by=1080)), .combine = 'rbind') %dopar% {
  ...
  rqscale1<-rqscale(x,interval=9,fast=TRUE,batch=1000,boot=TRUE,subsample=540,sorted=TRUE)
  rqtm1<-rqtm(x,interval=9,fast=TRUE,batch=1000,boot=TRUE,subsample=540,sorted=TRUE)
  rqfm1<-rqfm(x,interval=9,fast=TRUE,batch=1000,boot=TRUE,subsample=540,sorted=TRUE)
  targetm<-0
  targetvar<-(2)
  targettm<-0
  targetfm<-6*(sqrt(2)^4)
...
write.csv(allforlaplace,paste("Conlaplace.csv", sep = ","), row.names = TRUE)
...
write.csv(allfornorm,paste("Connorm.csv", sep = ","), row.names = TRUE)
...
write.csv(allforlogis,paste("Conlogis.csv", sep = ","), row.names = TRUE)
...
write.csv(allforRayleigh,paste("ConRayleigh.csv", sep = ","), row.names = TRUE)
...
write.csv(allforexp,paste("Conexp.csv", sep = ","), row.names = TRUE)
...
write.csv(listWeibull,paste("ConWeibull.csv", sep = ","), row.names = TRUE)
...
write.csv(listgamma,paste("Congamma.csv", sep = ","), row.names = TRUE)
...
write.csv(listlnorm,paste("Conlnorm.csv", sep = ","), row.names = TRUE)
...
write.csv(listpareto,paste("Conpareto.csv", sep = ","), row.names = TRUE)